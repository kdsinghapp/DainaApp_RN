...
const DeliveryHome = () => {
  ...
  const ListHeaderComponent = useCallback(() => (
    <>
      ...
      <View style={styles.container1}>
        <ReAnimated.View entering={FadeInDown.delay(100).duration(500)} style={styles.card1}>
          <Image source={imageIndex.cars} style={{ height: 35, tintColor: "#7B3F00", width: 35 }} resizeMode="contain" />
          <Text style={styles.title}>{strings.PendingRides}</Text>
          <Text style={{ color: "black", fontSize: 18, fontFamily: font.MonolithRegular, marginTop: 5 }}>
            {counts?.pendingRides || "0"}
          </Text>
        </ReAnimated.View>
        <ReAnimated.View entering={FadeInDown.delay(200).duration(500)} style={styles.card1}>
          <Image source={imageIndex.cars} style={{ height: 35, width: 35, tintColor: "#7B3F00" }} resizeMode="contain" />
      ...
      <View style={styles.container1}>
        <ReAnimated.View entering={FadeInDown.delay(300).duration(500)} style={styles.card1}>
          <Image source={imageIndex.earing} style={{ height: 35, width: 35, tintColor: "#7B3F00" }} resizeMode="contain" />
          <Text style={styles.title}>{strings.TotalEarnings}</Text>
          <Text style={{ color: "black", fontSize: 18, fontFamily: font.MonolithRegular, marginTop: 5 }}>
            {counts?.totalEarnings || "0.00"} ₮
          </Text>
        </ReAnimated.View>
        <ReAnimated.View entering={FadeInDown.delay(400).duration(500)} style={styles.card1}>
          <Image source={imageIndex.cars} style={{ height: 35, tintColor: "#7B3F00", width: 35 }} resizeMode="contain" />
  ...
  ), [coords, isOnline, setIsOnline, counts]);
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Animated.View style={{ flex: 1, transform: [{ translateX }], opacity: fade, marginBottom: 70 }}>
        <FlatList
